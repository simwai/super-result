 ...
 * @category Capture
 */
export async function onFinallyAsync<T, E, FE = unknown>(
	...
	const res = onFinally(
		result,
		callback as (result: Result<T, E>) => void,
		mapFinallyError,
	);
	return (isPromiseLike(res) ? res : Promise.resolve(res)) as ResultAsync<
		T,
		E | FinallyError<T, E, FE>
	>;
}
...
/**
 * Chain another operation that returns a {@link Result}.
 *
 * @category Transformers
 */
...
/**
 * Async variant of {@link flatMap}. Accepts both `Result` and `ResultAsync`.
 *
 * @category Transformers
 */
		...
		if (opts?.finally) {
			return onFinally(
				...
				opts.mapFinallyError ??
					(mapFinallyError as unknown as (e: unknown) => G),
			);
		}
		return result;
	}
