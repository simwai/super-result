...
export function map<T, E, U>(input: RawResult<T, E>, fn: (v: T) => U): RawResult<U, E>;
export function map<T, E, U>(input: Promise<RawResult<T, E>>, fn: (v: T) => U): Promise<RawResult<U, E>>;
...
export function flatMap<T, E, U>(input: RawResult<T, E>, fn: (v: T) => RawResult<U, E>): RawResult<U, E>;
export function flatMap<T, E, U>(input: Promise<RawResult<T, E>>, fn: (v: T) => RawResult<U, E>): Promise<RawResult<U, E>>;
...
export function onFinally(input: any, callback: any, mapFinallyError?: any): any {
  const mapper = mapFinallyError ?? ((e: unknown) => e);
  const toErrResult = (original: RawResult<any, any>, err: unknown): RawResult<any, any> => {
  ...
  try {
    ...
    if (isPromise(res)) {
      return res.then(() => input, (e) => toErrResult(input, e));
    }
    return input;
  } catch (e) {
...
export function unwrapOr<T, E, D>(input: RawResult<T, E>, defaultValue: D): T | D;
export function unwrapOr<T, E, D>(input: Promise<RawResult<T, E>>, defaultValue: D): Promise<T | D>;
...
export function unwrapOrElse<T, E, D>(input: RawResult<T, E>, fallback: (e: E) => D): T | D;
export function unwrapOrElse<T, E, D>(input: Promise<RawResult<T, E>>, fallback: (e: E) => D): Promise<T | D>;
...
export async function fromPromise<T, E>(
  ...
  try { return ok(await promise); } catch (e) { return err(mapError(e)); }
}
...
export function result<T, E>(promise: Promise<RawResult<T, E>>): Promise<RawResult<T, E>>;
export function result<T>(fn: () => T): RawResult<T, unknown>;
export function result<T, E>(fn: () => Promise<T>, mapError?: (e: unknown) => E): Promise<RawResult<T, E>>;
export function result(input: any, mapError?: (e: unknown) => any): any {
  if (typeof input === "function" && input.length === 0) {
    try {
      const res = input();
      if (isPromise(res)) {
        return res.then(v => ok(v), e => err(mapError ? mapError(e) : e));
      }
      return ok(res);
    } catch (e) {
  ...
  if (isPromise(input)) {
    return input.then(v => (isRawResult(v) ? v : ok(v)), e => err(mapError ? mapError(e) : e));
  }
...
export function createResult<E = unknown>(
  options?: ((error: unknown) => E) | { mapError?: (error: unknown) => E; mapFinallyError?: (error: unknown) => unknown }
) {
  const mapError = typeof options === "function" ? options : (options?.mapError ?? ((e: unknown) => e as E));
  const mapFinallyError = typeof options === "function" ? (e: unknown) => e : (options?.mapFinallyError ?? ((e: unknown) => e));
  return {
    ok,
    err: (e: E) => err(e),
    fromThrowable: <T>(fn: () => T) => {
      try { return ok(fn()); } catch (e) { return err(mapError(e)); }
    },
    fromPromise: <T>(p: PromiseLike<T>) => fromPromise(p, mapError),
    result: <T>(input: any) => result(input, mapError),
    map,
    flatMap,
    onFinally: (input: any, callback: any) => onFinally(input, callback, mapFinallyError),
    unwrap,
    unwrapSync,
    unwrapOr,
    unwrapOrElse,
